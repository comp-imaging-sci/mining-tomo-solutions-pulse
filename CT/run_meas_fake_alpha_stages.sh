...
LIMITED_ANGLE=1
NOISE_LAYERS=5
#STAGES=(5000,2500,2500)
if [[ $LIMITED_ANGLE -eq 1 ]]
then
    N_PROJ=120
    #N_PROJ=1
    INPUT_DIR=input_airt_y_fake_${IDX}_I_${I}/la_${N_PROJ}
    OUTPUT_DIR=/shared/planck/MRI/sbhadra/pulse_ct/runs/fake_alpha_I_${I}/la_${N_PROJ}/stages_4_layers_${NOISE_LAYERS}_fake_${IDX}_lr_${LEARNING_RATE}_p_${P}_reg_${LAMBDA}
    #OUTPUT_DIR=/shared/planck/MRI/sbhadra/pulse_ct/runs/fake_alpha_I_${I}/la_${N_PROJ}/stages_2_cosine_layers_${NOISE_LAYERS}_fake_${IDX}_lr_${LEARNING_RATE}_p_${P}_reg_${LAMBDA}
    #OUTPUT_DIR=/scratch/pulse_ct_stages/
else
    N_PROJ=24
...
then
    python -u run_airt_alpha_kl.py -limited_angle -I $I -n_proj $N_PROJ -p $P -input_dir $INPUT_DIR -output_dir $OUTPUT_DIR -duplicates $DUPLICATES -lr_schedule $LR_SCHEDULE -loss_str $LOSS_STR -loss_domain meas -eps $EPS \
            ...
            -num_trainable_noise_layers $NOISE_LAYERS -stages 1000 500 500 
else
