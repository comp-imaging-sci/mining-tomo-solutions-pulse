...
fanbeam_angles = np.linspace(0, 2*np.pi, args.n_angles, endpoint=False)
radon_fanbeam = RadonFanbeam(512,fanbeam_angles,source_distance=512, det_distance=512, det_spacing=3.0)

# Add Poisson noise if noise in measurement data
def noisy_sinogram(g_bar,I):
...
for im in Path(args.input_dir).glob("*.*"):
    ...
    if args.I is not None:
        sinogram = noisy_sinogram(sinogram,args.I)
    sinogram_np = sinogram.detach().clone().cpu().numpy()
    np.save(Path(output_dir) / (im.stem+f"_{im_count}.npy"),sinogram_np)
    im_count += 1
