...
class PULSE(torch.nn.Module):
    ...
    def forward(self, ref_im,
        ...
        #print(ref_im.shape)
        loss_builder = LossBuilder(ref_im, self.n_angles, loss_str, loss_domain, eps).cuda()
        ...
        for j in range(steps):
            ...
            if(save_intermediate):
                ...
                yield (gen_im.cpu().detach(),loss_builder.D(gen_im).cpu().detach())
                #yield (best_im.cpu().detach().clamp(0, 1),loss_builder.D(best_im,mask).cpu().detach().clamp(0, 1))
        ...
        if(min_l2 <= eps):
            ...
            yield (best_im.clone().cpu().detach(),loss_builder.D(best_im).cpu().detach())
        else:
