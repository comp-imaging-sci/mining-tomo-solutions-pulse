...
import time
import torch
from random_loss import LossBuilder
...
class PULSE_random_alpha(torch.nn.Module):
    ...
    def __init__(self, cache_dir, model_name, num_w_layers, I, n_proj, better_fit=False, verbose=True):
        super(PULSE_random_alpha, self).__init__()
    ...
    def forward(self, ref_im,
        ...
        num_meas = ref_im.shape[2]

        # Compute number of measurements for random data fidelity
        num_random = int(np.floor(random_fraction*torch.numel(ref_im)))
        ...
        def lambda_rule(x,steps,num_random):
            if x < num_random:
                if x < 0.9*num_random:
                    x = (9*(1-np.abs(x/(0.9*num_random)-1/2)*2)+1)/10
                else:
                    x = 1/10 + (x-0.9*num_random)/(0.1*num_random)*(1/1000-1/10)
            elif x == num_random:
                x = 1
            else:
                if x < 0.9*(steps-num_random):
                    x = (9*(1-np.abs(x/(0.9*(steps-num_random))-1/2)*2)+1)/10
                else:
                    x = 1/10 + (x-0.9*(steps-num_random))/(0.1*(steps-num_random))*(1/1000-1/10)
            return x
        ...
        schedule_dict = {
            ...
            #'linear1cycledrop': lambda x: lambda_rule(x,steps,num_random),
        }
        ...
        loss_builder = LossBuilder(ref_im, loss_str, loss_domain, self.I, self.n_proj, mu_max, H_csc, eps).cuda()
        ...
        for j in range(steps):
            ...
            # Calculate Losses
            if j==random_epochs:
                print('\nSwitching to full data fidelity')
            if j<random_epochs:
                random_indices = torch.randperm(num_meas)[:num_random]
            else:
                random_indices = torch.arange(num_meas)
            random_indices = random_indices.cuda()
            loss, loss_dict = loss_builder(latent_in, noise_vars, gen_im, random_indices)
            ...
            if j%save_interval==0:
                print('Iter =  %d, Total loss = %.3f, KL loss = %.3f'%(j,loss,loss_kl))
