...
f_pinv_t, loss = cgne(radon_fanbeam,x_zero,g,callback=lambda xx: (torch.norm(radon_fanbeam.forward(xx)-g)**2).item(),max_iter=200,tol=1e-2)
f_pinv = f_pinv_t.detach().cpu().numpy()
#f_null = f_np - f_meas
#f_null_t = torch.FloatTensor(f_null).to(device)
#Hf_null = radon_fanbeam.forward(f_null_t).cpu().numpy()

plt.figure(); plt.imshow(f_pinv,cmap='gray'); plt.colorbar(); plt.title(r'$f_{pinv}$')
plt.savefig('./results_cgne_pinv/f_pinv_cgne.png',bbox_inches='tight')
# plt.figure(); plt.imshow(f_null,cmap='gray'); plt.colorbar(); plt.title(r'$f_{null}$')
# plt.savefig('./results_cgne_pinv/f_null_cgne.png',bbox_inches='tight')
...
plt.savefig('./results_cgne_pinv/convergence_plot_cgne.png',bbox_inches='tight')
plt.figure(); plt.semilogy(loss); plt.xlabel('Iterations'); plt.ylabel('Loss') #plt.ylabel(r'$\log ||g-Hf||^2$')
plt.savefig('./results_cgne_pinv/semilog_convergence_plot_cgne.png',bbox_inches='tight')
