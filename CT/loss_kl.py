...
class LossBuilder(torch.nn.Module):
    ...
    def __init__(self, ref_im, loss_str, loss_domain, I, n_angles, eps):
        super(LossBuilder, self).__init__()
        #assert ref_im.shape[2]==ref_im.shape[3]
        assert ref_im.shape[2] == n_angles
        im_size = ref_im.shape[3]
        self.D = projector(loss_domain=loss_domain,n_angles=n_angles)
        self.I = I
        self.ref_im = ref_im
    ...
    def _loss_kl(self, gen_im_lr, ref_im, **kwargs):
        loss_val = torch.sum(ref_im*gen_im_lr) + torch.sum(self.I * torch.exp(-gen_im_lr))
        return loss_val
    ...
    def forward(self, latent, noise_vars, gen_im):
