...
class LossBuilder(torch.nn.Module):
    ...
    def __init__(self, ref_im, loss_str, loss_domain, I, n_proj, mu_max, H_csc, df_constant, eps):
    ...
    def _loss_kl(self, gen_im_lr, ref_im, **kwargs):
        loss_val = torch.sum(ref_im*gen_im_lr) + self.I * torch.sum(torch.exp(-gen_im_lr)) + self.df_constant
        return loss_val
