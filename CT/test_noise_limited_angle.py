...
angle_factor = 3
n_angles = 512//angle_factor
device = torch.device('cuda')
fanbeam_angles = np.linspace(0, 2*np.pi/angle_factor, n_angles, endpoint=False)
#fanbeam_angles = np.linspace(0, 270, 384, endpoint=False)
...
filtered_g_bar = radon_fanbeam.filter_sinogram(g_bar,filter_name='hann')
...
im3 = ax[1,1].imshow(fbp_np,cmap='gray'); fig.colorbar(im3,ax=ax[1,1],fraction=0.046,pad=0.04); ax[1,1].axis('off'); ax[1,1].set_title('FBP from 'r'$y$')
