...
FRACTION=0.99
LEARNING_RATE=0.4
LAMBDA=0 # GEOCROSS loss hyperparameter
...
#OUTPUT_DIR=/shared/aristotle/MRI/sbhadra/pulse_null_functions/runs/noisy_snr_${SNR}_meas_AR/${MASK_TYPE}/bf_lr_${LEARNING_RATE}_frac_${FRACTION}_geocross_$LAMBDA
OUTPUT_DIR=/shared/aristotle/MRI/sbhadra/pulse_null_functions/runs/noisy_snr_${SNR}_meas_AR/${MASK_TYPE}/bf_test
DUPLICATES=200
...
python -u run.py -input_dir $INPUT_DIR -output_dir $OUTPUT_DIR -duplicates $DUPLICATES -loss_str $LOSS_STR -loss_domain meas -eps 1 -fraction $FRACTION -better_fit \
