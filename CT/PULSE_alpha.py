...
class PULSE_alpha(torch.nn.Module):
    ...
    def __init__(self, cache_dir, model_name, num_w_layers, n_angles, better_fit=False, verbose=True):
    ...
    def forward(self, ref_im,
        ...
        loss_builder = LossBuilder(ref_im, loss_str, loss_domain, self.n_angles, eps).cuda()
        ...
        for j in range(steps):
            ...
            # Normalize image to [0,0.01] instead of [-1,1]
            gen_im = 1e-2 * (self.synthesis(latent_in, noise)+1)/2
            ...
            if(save_intermediate):
                ...
                yield (gen_im.cpu().detach(),loss_builder.D(gen_im).cpu().detach(),best_im.cpu().detach(),total_loss_array,loss_l2_array)
                #yield (best_im.cpu().detach().clamp(0, 1),loss_builder.D(best_im,mask).cpu().detach().clamp(0, 1))
