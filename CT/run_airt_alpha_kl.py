...
from PULSE_airt_alpha_kl import PULSE_airt_alpha_kl
#from PULSE_alpha_stages import PULSE_alpha_stages
...
parser.add_argument('-mu_max', type=float, default=0.08, help='Maximum attenuation coefficient')
parser.add_argument('-df_constant', type=float, default=-100, help='Constant term in the data fidelity')
...
parser.add_argument('-stages', type=int, nargs='+', default=[5000,2500,2500], help='Optimize in stages following ILO')
...
parser.add_argument('-stop_discrepancy', action='store_true', help='Whether to stop optimization if discrepancy tolerance is satisfied')
...
parser.add_argument('-n_proj', required=True, type=int, default=100, help='Number of projections per rotation')
...
if not kwargs["better_fit"]:
    model = PULSE_airt_alpha_kl(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],I=kwargs["I"],n_proj=kwargs["n_proj"])
    #model = PULSE_alpha_stages(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],I=kwargs["I"],n_proj=kwargs["n_proj"])
else:
    model = PULSE_bf_alpha(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],n_proj=kwargs["n_proj"])
