...
LEARNING_RATE=0.4
C=1e-4 # Used in paper for 6-fold mask
#C=1e-3 # Used in paper for 8-fold mask
LAMBDA=0.01 # Euclidean loss hyperparameter
...
OUTPUT_DIR=runs_pp/${MASK_TYPE}/lr_${LEARNING_RATE}_p_${P}_reg_${LAMBDA}
DUPLICATES=100
STEPS=10000
SAVE_INTERVAL=100
MODEL_NAME='MRI'
...
python -u run_pp.py -sigma $SIGMA -c $C -input_dir $INPUT_DIR -output_dir $OUTPUT_DIR -duplicates $DUPLICATES -loss_str $LOSS_STR -loss_domain meas -eps $EPS \
