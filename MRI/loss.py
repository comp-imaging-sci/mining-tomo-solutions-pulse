...
class LossBuilder(torch.nn.Module):
    ...
    #def __init__(self, ref_im, mask, loss_str, loss_domain, eps, sigma, alpha_min, alpha_max):
    def __init__(self, ref_im, mask, loss_str, loss_domain, eps, sigma):
        super(LossBuilder, self).__init__()
        if loss_domain == 'image':
    ...
    def _loss_l2(self, gen_im_lr, ref_im, **kwargs):
        ...
        #loss_val = ((gen_im_lr - ref_im).pow(2).mean((1, 2, 3)).clamp(min=0).sum())
    ...
    # log-barrier loss
    def _loss_log_barrier(self, latent, **kwargs):
        loss_val = 0
        for layer in range(latent.shape[1]):
            #print('norm of v_i = '+str(torch.norm(latent[:,layer,:],2)))
            loss_val = loss_val -torch.log((torch.norm(latent[:,layer,:],2)-self.alpha_min)) - torch.log((self.alpha_max-torch.norm(latent[:,layer,:],2)))
            print('LB loss = '+str(loss_val))
        return loss_val
    ...
    def forward(self, latent, noise_vars, gen_im, mask):
        ...
        loss_fun_dict = {
            ...
            'LOG_BARRIER': self._loss_log_barrier,
        }
        losses = {}
