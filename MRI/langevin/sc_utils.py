...
# SB: Singlecoil forward operator for MRI
class SinglecoilForwardMRI(nn.Module):
    def __init__(self):
        super(SinglecoilForwardMRI, self).__init__()
        return
    ...
    def forward(self, image, mask):
        ...
        # Convert to k-space data
        ksp_coil = self._fft(image)

        # Apply sampling mask
        ksp_coil = mask * ksp_coil
...
# SB: get_mvue for singlecoil MRI
def get_mvue(kspace):
    ''' Get mvue estimate from coil measurements '''
    return np.real(np.fft.ifft2(kspace,axes=(-1,-2),norm='ortho'))
