...
#from PULSE_bf_alpha import PULSE_bf_alpha
#from PULSE_alpha_lbfgs import PULSE_alpha_lbfgs
#from PULSE_fista import PULSE_fista
#from PULSE_ip import PULSE_ip
...
parser.add_argument('-sigma', type=float, default=1, help='Measurement noise std')
...
parser.add_argument('-bad_noise_layers', type=str, default="13", help='List of noise layers to zero out to improve image quality')
...
parser.add_argument('-num_ranges', type=int, default=5, help='Number of hollow ball radii ranges to vary over the ECDF of whitened latent vector norm')
parser.add_argument('-range_idx', type=int, default=0, help='The range index for the hollow ball radii')
...
parser.add_argument('-stop_morozov', action='store_true', help='Whether to stop optimization if Morozov tolerance is satisfied')
...
parser.add_argument('-mask_type', type=str, required=True, default='mask_random_8_fold_cartesian_256x256', help='Type of mask')
...
if not kwargs["better_fit"]:
    model = PULSE_alpha(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],mask_type=kwargs["mask_type"])
    #model = PULSE_alpha_lbfgs(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],mask_type=kwargs["mask_type"])
    #model = PULSE_fista(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],mask_type=kwargs["mask_type"])
    #model = PULSE_ip(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],mask_type=kwargs["mask_type"])
else:
    model = PULSE_bf_alpha(cache_dir=kwargs["cache_dir"],model_name=kwargs["model_name"],num_w_layers=kwargs["num_w_layers"],mask_type=kwargs["mask_type"])
...
for ref_im, ref_im_name in dataloader:
    ...
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR,best_HR,total_loss,loss_l2) in enumerate(model(ref_im,**kwargs)):
            for i in range(kwargs["batch_size"]):
                np.save(out_path / ref_im_name[i] / "best_HR.npy",best_HR)
                np.save(out_path / ref_im_name[i] / "total_loss.npy",total_loss)
                np.save(out_path / ref_im_name[i] / "loss_l2.npy",loss_l2)
    ...
    #             #     out_path / f"{ref_im_name[i]}.png")
        ...
        for j,(best_HR,total_loss,loss_l2) in enumerate(model(ref_im,**kwargs)):
            for i in range(kwargs["batch_size"]):
                ...
                np.save(int_path / "loss_l2.npy",loss_l2)
