...
class PULSE_alpha(torch.nn.Module):
    ...
    def __init__(self, cache_dir, model_name, num_w_layers, mask_type, better_fit=False, verbose=True):
        ...
        #    pass
            ...
            mapping.load_state_dict(torch.load(self.model_name+'_mapping.pt'))
    ...
    def forward(self, ref_im,
        ...
        alpha_min_vals = pickle.load(open('no_bf_alpha_min_2.pkl','rb'))
        alpha_max_vals = pickle.load(open('no_bf_alpha_max_2.pkl','rb'))
        ...
        if(tile_latent):
            ...
            # v_plus = np.load('./G_range_object/latent_0_1_nf.npy')
            # v_plus = v_plus[:,0,:]
            # latent = torch.from_numpy(v_plus).float().view(1,1,512).to('cuda'); latent.requires_grad=True
        else:
            latent = torch.randn(
                (batch_size, self.num_w_layers, 512), dtype=torch.float, requires_grad=True, device='cuda')
            # dlatents = np.load('./G_range_object/dlatents.npy')
            # v_plus = np.load('./G_range_object/latent_0_1.npy')
            # latent = torch.from_numpy(v_plus).float().to('cuda'); latent.requires_grad=True
            # latent = torch.zeros(
            #     (batch_size, self.num_w_layers, 512), dtype=torch.float, requires_grad=True, device='cuda')
        ...
        opt_dict = {
            ...
            #'adam': partial(torch.optim.Adam, lr=learning_rate),
            'rmsprop': torch.optim.RMSprop,
            'sgdm': partial(torch.optim.SGD, momentum=0.9),
            'adamax': torch.optim.Adamax
        }
        opt_func = opt_dict[opt_name]
        opt = HollowBallOptimizerDelta(opt_func, var_list, delta_min=alpha_min, delta_max=alpha_max, lr=learning_rate)
        ...
        #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt.opt,steps)
        ...
        for j in range(steps):
            ...
            #latent_norm_array = latent_norm_array.append(latent_norm_max)
            ...
            #gen_im = torch.clamp(gen_im,min=0,max=1)

            # Check latent and initial loss (for debugging)
            if j==0:
                # diff_latent = dlatents - latent_in.detach().cpu().numpy()
                # print('Difference in latents = '+str((diff_latent**2).sum()))
                gen_meas = loss_builder.D(gen_im,mask)
                l2_loss = 0.5*torch.norm(gen_meas-ref_im)**2/sigma**2
                print('Initial loss = '+str(l2_loss.item()))
            ...
            if j%save_interval==0:
                #print('\nLatent norm (max) = '+str(latent_norm_max.item()))
