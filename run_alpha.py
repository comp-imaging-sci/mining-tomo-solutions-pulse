...
for ref_im, ref_im_name in dataloader:
    ...
    if(kwargs["save_intermediate"]):
        ...
        for j,(HR,LR,current_grad_norm,total_loss,loss_l2,w_latent_active,noise_latent_active) in enumerate(model(ref_im,**kwargs)):
            for i in range(kwargs["batch_size"]):
                np.save(out_path / ref_im_name[i] / "current_grad_norm.npy",current_grad_norm)
                np.save(out_path / ref_im_name[i] / "total_loss.npy",total_loss)
                np.save(out_path / ref_im_name[i] / "loss_l2.npy",loss_l2)
                np.save(out_path / ref_im_name[i] / "w_latent_active.npy",w_latent_active)
                np.save(out_path / ref_im_name[i] / "noise_latent_active.npy",noise_latent_active)
